/**
	 ...
	 */
	 ...
	 void conHelper(vector<Point<Dim>>& newPoints, int left, int right, int dimension, KDTreeNode*& subroot);
	 int partition(vector<Point<Dim>>& vec, int left, int right, size_t index, int dimension);
	 Point<Dim> select(vector<Point<Dim>>& vec, int left, int right, unsigned long median, int dimension);
	 void nearestNeighborHelper(KDTreeNode* curr, Point<Dim> &currentBest, Point<Dim>& query, int dimension) const;
	 private:
	 ...
	 */
	 ...
	 void copy_(KDTreeNode* subroot, KDTreeNode * othersubroot);
	};
